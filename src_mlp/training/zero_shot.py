        ...
        for images, target in tqdm(dataloader, unit_scale=args.batch_size):
            ...
            with autocast():
                ...
                if args.moe_head == 1:
                    ...
                    image_features = image_features.reshape(-1, args.moe_head, image_features.shape[-1]).transpose(0, 1)  # (moe_head,bs,512)
...
def zero_shot_eval(model, data, epoch, args, tokenizer=None):
    if 'imagenet-val' not in data and 'imagenet-v2' not in data and 'imagenet-r' not in data and 'imagenet-a' not in data:
        return {}
