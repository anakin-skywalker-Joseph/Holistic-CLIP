...
class ClipLoss(nn.Module):
    ...
    def logits_assign_hard(self, image_feature, text_feature, logit_scale, return_both=True, return_text=False, device=None):
        # image_feature: (cls_num, batch_size, embed_dim)
        ...
        if image_feature.shape[0] == text_feature.shape[0]:
            for i in range(image_feature.shape[0]):
                logits = logit_scale * image_feature[i] @ text_feature[i].T #(bs_img, bs_text)
                if return_both:
                    text_feature_concat = torch.cat((text_feature[i:i+1],text_feature[:i],text_feature[i+1:]),dim=0) # (num_caption, bs_text, embed_dim)

                    text_feature_concat = text_feature_concat.reshape(-1, text_feature.shape[-1]) # (bs_text*num_caption, embed_dim)
    ...
    def get_logits(self, image_features, text_features, logit_scale, device = None):
        ...
        if len(text_features.shape) == 2:  # one caption
            image_features = image_features.reshape(-1, self.cls_num, image_features.shape[-1]) # (batch_size, cls_num, embed_dim)
            if self.world_size > 1:
                all_image_features, all_text_features = gather_features(
                    image_features, text_features,
                    self.local_loss, self.gather_with_grad, self.rank, self.world_size, self.use_horovod)
                if self.local_loss:
                    image_features = image_features.transpose(0,1) # (cls_num, batch_size, embed_dim)
                    all_image_features = all_image_features.transpose(0,1) # (cls_num, batch_size, embed_dim)
    ...
    def forward(self, image_features, text_features, logit_scale, output_dict=False, epoch=10):
        ...
        if len(logits_per_image.shape) == 2: # one caption
            ...
            if logits_per_image_full is None:
                ...
                label_list = [labels + i*all_batch_size for i in range(self.cls_num)]
                contrastive_loss = (
                    0.99 * F.cross_entropy(logits_per_image, labels) +
                    0.99 * F.cross_entropy(logits_per_text, labels)) / 2
                #label_list[i] set i to 0 if not use
        ...
        # contrast between cls image token
        rank_loss = torch.tensor(0.0,device=device)
        if self.cls_num >1 and epoch < 200: #not used
            image_features = image_features.reshape(-1, self.cls_num, image_features.shape[-1])
            moe_sim_matrix = image_features @ image_features.transpose(1,2) # (bs, cls_num, cls_num)
